...
public final class CreateContentHelper {
	...
	public static void saveDataToDb(
		...
		for (Integer answerKey : answerBundle.keySet()) {
			QuizContentDAO quizContent =
					addQuizContent(quizId, quesId, questionText, answerBundle, score, answerKey);
			String queryString = convertToQueryString(quizContent);
			DataOps.saveData(queryString);
		}
	}

	public static QuizContentDAO addQuizContent(
			int quizId,
			int quesId,
			String questionText,
			Map<Integer, ArrayList<String>> answerBundle,
	...
	public static List<QuizFormDAO> processPayload(List<QuizQuestionsDAO> quizQuestionsList) {
		...
		for (QuizQuestionsDAO question : quizQuestionsList) {
			...
			answerBundle.put(
					...
					new ArrayList<>(Arrays.asList(question.getOptionA(), choice == 1 ? "true" : "false")));
			answerBundle.put(
					generateRandom(10000, 99999),
					new ArrayList<>(Arrays.asList(question.getOptionB(), choice == 2 ? "true" : "false")));
			answerBundle.put(
					generateRandom(10000, 99999),
					new ArrayList<>(Arrays.asList(question.getOptionC(), choice == 3 ? "true" : "false")));
			answerBundle.put(
					generateRandom(10000, 99999),
					new ArrayList<>(Arrays.asList(question.getOptionD(), choice == 4 ? "true" : "false")));
