...
public class StudentAnswerModel{
	...
	public String storeResponses(String studentResponse) {
		...
		Quiz quiz = new Quiz(quizId, jsonResponse.getQuizTitle(), jsonResponse.getQuizInstruct(),
				jsonResponse.getQuizType(), jsonResponse.getTimeLimit());
		for (QuestionDetails questionMapper : questions) {
			int questionId = questionMapper.getQuestionId();
			Question question = new Question(quiz, questionId, questionMapper.getQuestion(), questionMapper.getMarks());
			...
			if (answers != null) {
				for (AnswerDetails ansMapper : answers) {
					...
					statR.insertStudentResponse(stats);
				}
			}
		}
	...
	public int fetchQuizId(String quizName){
		QuizEntity quizEntity = new QuizEntity();
		return quizEntity.fetchQuizId(quizName);
	}
	...
	public List<String> fetchQuizStatus(List<String> quizNames){
		...
		for(String quizName : quizNames) {
			...
			long count = statR.checkQuizStatus(quizID);
			if(count>=1){
				status.add("Answered");
			}
			else {
