...
public class ViewQuizServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		boolean graded = true;
		try {
			// The information in the following items will be placed securely on a seperate
			// file.
			...
			// Query strings to gets entire question info and only the isGraded boolean from
			// Quiz table
			PreparedStatement query;
			query = connection
					.prepareStatement("select A.*, B.isGraded, B.quizTitle, B.quizScheduledDate, B.quizInstruction "
							+ "from [dbo].[Questions] A " + "join [dbo].[Quiz] B " + "on A.quizId = B.quizId "
							+ "where A.quizId = ?");
			...
			while (result.next()) {
				...
				String points = result.getString("totalPoints");
				...
				QuestionsVO quest = new QuestionsVO(0, question, answer, choice1, choice2, choice3, points, graded);
